...
class ConvTransposeNet(nn.Module):
    ...
    def __init__(self,
        ...
        if hidden_layers == 0:
            ...
            layers.append(nn.ConvTranspose2d(in_channels, hidden_channels[0], kernel_size, 2, padding, output_padding=1, bias=bias))
            layers.append(act_layer())
            for i in range(1, hidden_layers - 1):
                layers.append(nn.ConvTranspose2d(hidden_channels[i - 1], hidden_channels[i], kernel_size, 2, padding, output_padding=1, bias=bias))
                layers.append(act_layer())
